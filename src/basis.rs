    ...
    F: Float,
{
    ...
    fn vandermonde(order: usize, points: ArrayView2<'_, F>) -> Array2<F> {
        Self::orthonormal_basis(order, points)
    }
    /// The inverted Vandermonde matrix at given points
    fn inverted_vandermonde(order: usize, points: ArrayView2<'_, F>) -> Array2<F> {
        Self::vandermonde(order, points)
            .inv()
            .expect("failed vandermonde inversion, check precision and polynomial order")
    }
    /// The nodal basis at given points
    fn nodal_basis_at(order: usize, points: ArrayView2<'_, F>) -> Array2<F> {
        Self::orthonormal_basis(order, points)
            .dot(&Self::inverted_vandermonde(order, points))
    }
    /// The derivative of the nodal basis at given points
    fn grad_nodal_basis(order: usize, points: ArrayView2<'_, F>) -> Array2<F> {
        todo!()
    }
}
...
impl<F: Float> Basis<F> for LineBasis<F> {
    ...
    fn grad_orthonormal_basis(&self, points: ArrayView2<'_, F>) -> Array2<F> {
        let half = F::from(0.5).unwrap();
